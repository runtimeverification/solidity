...
)
{
	...
	{
		abstract = true;
		var = make_shared<SymbolicIntVariable>(frontend::TypeProvider::uint(), type, _uniqueName, _context);
	}
	else if (isBool(_type))
	...
	else if (isFunction(_type))
	{
		...
		{
			abstract = true;
			var = make_shared<SymbolicIntVariable>(TypeProvider::uint(), type, _uniqueName, _context);
		}
		else
