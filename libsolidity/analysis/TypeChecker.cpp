...
bool TypeChecker::visit(FunctionCallOptions const& _functionCallOptions)
{
	...
	for (size_t i = 0; i < _functionCallOptions.names().size(); ++i)
	{
		...
		else if (name == "value")
		{
			...
			{
				expectType(*_functionCallOptions.options()[i], *TypeProvider::uint(256));
...
void TypeChecker::endVisit(NewExpression const& _newExpression)
{
	...
	else if (type->category() == Type::Category::Array)
	{
		...
		_newExpression.annotation().type = TypeProvider::function(
			TypePointers{TypeProvider::uint()},
			TypePointers{type},
			strings(1, ""),
			strings(1, ""),
...
bool TypeChecker::visit(MemberAccess const& _memberAccess)
{
	...
	if (exprType->category() == Type::Category::Magic)
	{
		...
		if (magicType.kind() == MagicType::Kind::Message)
		{
			if (memberName == "sig" || memberName == "data")
				m_errorReporter.typeError(2699_error, _memberAccess.location(), "msg." + memberName + " is not supported in IELE. For more information, including potential workarounds, see README-IELE-SUPPORT.md");
		}
	}
	...
	if (exprType->category() == Type::Category::Function)
	{
		if (memberName == "selector")
			m_errorReporter.typeError(2524_error, _memberAccess.location(), "Member \"" + memberName + "\" is not supported in IELE. For more information, including potential workarounds, see README-IELE-SUPPORT.md");
	}
	return false;
}
...
bool TypeChecker::visit(IndexAccess const& _access)
{
	...
	case Type::Category::Array:
	{
		...
		{
			expectType(*index, *TypeProvider::uint());
	...
	case Type::Category::TypeType:
	{
		...
		{
			bigint length = 1;
			if (expectType(*index, *TypeProvider::uint()))
			{
