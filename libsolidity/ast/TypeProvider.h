...
public:
	...
	static ArrayType const* array(DataLocation _location, Type const* _baseType, bigint const& _length);
	...
	static IntegerType const* integer(IntegerType::Modifier _modifier)
	{
		if (_modifier == IntegerType::Modifier::Unsigned)
			return m_uint.get();
		else
	...
	static IntegerType const* uint() { return integer(IntegerType::Modifier::Unsigned); }
	static IntegerType const* sint() { return integer(IntegerType::Modifier::Signed); }
...
private:
    ...
    static std::unique_ptr<IntegerType> const m_int;
    static std::unique_ptr<IntegerType> const m_uint;
