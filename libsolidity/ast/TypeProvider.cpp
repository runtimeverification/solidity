...
unique_ptr<IntegerType> const TypeProvider::m_int
    {make_unique<IntegerType>(IntegerType::Modifier::Signed)};

unique_ptr<IntegerType> const TypeProvider::m_uint
    {make_unique<IntegerType>(IntegerType::Modifier::Unsigned)};
...
Type const* TypeProvider::fromElementaryTypeName(ElementaryTypeNameToken const& _type, std::optional<StateMutability> _stateMutability)
{
	...
	case Token::Int:
		return integer(IntegerType::Modifier::Signed);
	case Token::UInt:
		return integer(IntegerType::Modifier::Unsigned);
	case Token::Fixed:
...
ArrayType const* TypeProvider::array(DataLocation _location, Type const* _baseType, bigint const& _length)
{
