...
)
{
	...
	return m_functionCollector.createFunction(functionName, [&]()
	{
		...
		bigint baseValue = _baseType.literalValue(nullptr);
...
string YulUtilFunctions::clearStorageRangeFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		...
		("functionName", functionName)
		("setToZero", storageSetToZeroFunction(_type.storageBytes() < 32 ? *TypeProvider::uint(256) : _type))
		...
		switch (_type.location())
		{
			case DataLocation::Storage:
			{
				...
				return Whiskers(R"(
					...
					("mul", overflowCheckedIntMulFunction(*TypeProvider::uint(256)))
					.render();
			}
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		if (_mappingType.keyType()->isDynamicallySized())
			...
			("hash", packedHashFunction(
				{&_keyType, TypeProvider::uint(256)},
				{_mappingType.keyType(), TypeProvider::uint(256)}
			))
			.render();
		else
		{
