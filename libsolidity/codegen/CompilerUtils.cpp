...
{
	unsigned numBytes = prepareMemoryStore(*TypeProvider::uint(256), true);
	if (numBytes > 0)
...
void CompilerUtils::storeInMemoryDynamic(Type const& _type, bool _padToWordBoundaries, bool _cleanup)
{
	...
	if (auto ref = dynamic_cast<ReferenceType const*>(&_type))
	{
		...
		storeInMemoryDynamic(*TypeProvider::uint(256), _padToWordBoundaries, _cleanup);
	}
		...
		if (type->category() == Type::Category::Array)
		{
			...
			else
			{
				...
				if (calldataType->isDynamicallySized())
				{
					...
					loadFromMemoryDynamic(*TypeProvider::uint(256), !_fromMemory);
					m_context << Instruction::SWAP1;
					...
					// retrieve length
					loadFromMemoryDynamic(*TypeProvider::uint(256), !_fromMemory, true);
	...
	for (size_t i = 0; i < _givenTypes.size(); ++i)
	{
		...
		if (targetType->isDynamicallySized() && !_copyDynamicDataInPlace)
		{
			...
			if (_givenTypes[i]->category() == Type::Category::StringLiteral)
			{
				...
				m_context << u256(size);
				storeInMemoryDynamic(*TypeProvider::uint(256), true);
				// stack: ... <end_of_mem'>
...
void CompilerUtils::storeStringData(bytesConstRef _data)
{
	...
	if (_data.size() <= 32)
	{
		for (unsigned i = 0; i < _data.size(); i += 32)
		{
			...
			storeInMemoryDynamic(*TypeProvider::uint(256));
		}
		m_context << Instruction::POP;
	}
	else
