...
void IRGeneratorForStatements::endVisit(FunctionCall const& _functionCall)
{
	...
	case FunctionType::Kind::Event:
	{
		...
		if (!event.isAnonymous())
			define(indexedArgs.emplace_back(m_context.newYulVariable(), *TypeProvider::uint(256))) <<
	...
	case FunctionType::Kind::ObjectCreation:
	{
		...
		IRVariable value = convert(*arguments[0], *TypeProvider::uint(256));
		define(_functionCall) <<
...
void IRGeneratorForStatements::endVisit(IndexAccess const& _indexAccess)
{
		...
		switch (arrayType.location())
		{
			...
			case DataLocation::Memory:
			{
				string const memAddress =
					...
					", " +
					expressionAsType(*_indexAccess.indexExpression(), *TypeProvider::uint(256)) +
					")";
	...
	else if (baseType.category() == Type::Category::FixedBytes)
	{
		...
		IRVariable index{m_context.newYulVariable(), *TypeProvider::uint(256)};
...
void IRGeneratorForStatements::endVisit(IndexRangeAccess const& _indexRangeAccess)
{
	...
	switch (arrayType.location())
	{
		case DataLocation::CallData:
		{
			...
			IRVariable sliceStart{m_context.newYulVariable(), *TypeProvider::uint(256)};
