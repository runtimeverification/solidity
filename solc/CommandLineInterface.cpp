...
{
	sout() <<
		"isolc, the solidity to IELE compiler commandline interface" <<
		endl <<
		"Version: " <<
...
bool CommandLineInterface::parseArguments(int _argc, char** _argv)
{
	...
	po::options_description desc((R"(isolc, the Solidity to IELE commandline compiler.

This program comes with ABSOLUTELY NO WARRANTY. This is free software, and you
are welcome to redistribute it under certain conditions. See 'isolc --)" + g_strLicense + R"('
for details.

Usage: isolc [options] [input_file...]
...
Example:
isolc --)" + g_argBinary + R"( -o /tmp/solcoutput dapp-bin=/usr/local/lib/dapp-bin contract.sol
...
void CommandLineInterface::outputCompilationResults()
{
	...
	for (string const& contract: contracts)
	{
		...
		if (m_args.count(g_argAsm) || m_args.count(g_argAsmJson))
		{
			...
			{
				sout() << "IELE assembly:" << endl << ret << endl;
			}
		}
